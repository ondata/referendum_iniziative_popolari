...
mlr --jsonl filter '${idDecStatoIniziativa.id}==2' then cut -f id,sostenitori then put '$data="'"${check_date}"'"' "${folder}"/../data/source.jsonl >>"${folder}"/../data/time_line.jsonl

# Rimuove duplicati dalla timeline e riordina per data e ID
mlr -I --jsonl uniq -a then sort -t data,id "${folder}"/../data/time_line.jsonl
